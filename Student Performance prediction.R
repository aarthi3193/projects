...
d1=read.table("D:/student-mat.csv",sep=",",header=TRUE)
d2=read.table("D:/student-por.csv",sep=",",header=TRUE)

# one of the attributes, "paid," is course specific rather than student specific, so I am eliminating it from the list of attributes by which student
# that are matched
d.final=merge(d1,d2,by=c("school","sex","age","address","famsize","Pstatus",
...
d.final$mathgrades=rowMeans(cbind(d.final$G1.x,d.final$G2.x,d.final$G3.x))
d.final$portgrades=rowMeans(cbind(d.final$G1.y,d.final$G2.y,d.final$G3.y))

d.final$Dalc <- as.factor(d.final$Dalc)      
d.final$Dalc <- plyr::mapvalues(d.final$Dalc, 
                              from = 1:5, 
                              to = c("Very Low", "Low", "Medium", "High", "Very High"))

str1=ggplot(d.final, aes(x=mathgrades, y=portgrades)) +
  geom_point(aes(colour=factor(Dalc)))+ scale_colour_hue(l=25,c=150)+
  geom_smooth(method = "lm", se = FALSE)

d.final$Walc <- as.factor(d.final$Walc)      
d.final$Walc <- plyr::mapvalues(d.final$Walc, 
                              from = 1:5, 
                              to = c("Very Low", "Low", "Medium", "High", "Very High"))

str2=ggplot(d.final, aes(x=mathgrades, y=portgrades))+
